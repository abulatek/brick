...
def plot_spectra(spectrum_contsub, modeldata_all, transenergies_all, MolfitsFileNames):
    ...
    plt.title(f"{freq_spw} data and model comparison (outflow), $T$ = {tkin:.1f} K, $\log_{{10}}(N_{{tot}})$ = {np.log10(Ntot):.2f}") # y = 0.92 # NOTE: TKIN AND NTOT MAY CHANGE WHEN WE FIT !
    plt.savefig(f"/blue/adamginsburg/abulatek/brick/first_results/lots_of_plotting/all/spectra_ID/{freq_spw}_CE.pdf", facecolor = 'w', edgecolor = 'w', bbox_inches = 'tight', overwrite = True)
    plt.savefig(f"/blue/adamginsburg/abulatek/brick/first_results/lots_of_plotting/all/spectra_ID/{freq_spw}_CE.png", dpi = 250, bbox_inches = 'tight', overwrite = True)
