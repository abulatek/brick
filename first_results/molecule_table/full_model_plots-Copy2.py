...
def plot_full_model(loc, moltbl_fn, figdir, specfns, outdir, write_models=False, plot_noise_range=True, save_figs=False, show_figs=True, export_tbl=False):
    ...
    freq_centers_arr = []
    sigma_arr = []
    for spec in tqdm(spectra_inorder[0:2]):
...
def plot_spectrum(loc, spec, mod, name_labels, freq_labels, plot_noise_range, save_figs, show_figs): # v_cen_labels
    ...
    if save_figs:
        if loc == 'core':
            plt.savefig(f'data_model_c_{np.min(spec.xarr.to(u.GHz)).value:07.3f}_{np.max(spec.xarr.to(u.GHz)).value:07.3f}_test.png', bbox_inches='tight')
            plt.savefig(f'data_model_c_{np.min(spec.xarr.to(u.GHz)).value:07.3f}_{np.max(spec.xarr.to(u.GHz)).value:07.3f}_test.pdf', bbox_inches='tight')
        elif loc == 'frown':
            plt.savefig(f'data_model_f_{np.min(spec.xarr.to(u.GHz)).value:07.3f}_{np.max(spec.xarr.to(u.GHz)).value:07.3f}_test.png', bbox_inches='tight')
            plt.savefig(f'data_model_f_{np.min(spec.xarr.to(u.GHz)).value:07.3f}_{np.max(spec.xarr.to(u.GHz)).value:07.3f}_test.pdf', bbox_inches='tight')
    if show_figs:
        plt.show()
...
plt.savefig('line_density_proposal_test.pdf', facecolor = 'w', edgecolor = 'w', bbox_inches = 'tight')
plt.savefig('line_density_proposal_test.png', dpi = 250, bbox_inches = 'tight')
plt.show()
