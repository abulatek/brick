...
def plot_full_model(loc, moltbl_fn, figdir, specfns, outdir, write_models=False, plot_noise_range=True, save_figs=False, show_figs=True, export_tbl=False):
    ...
    freq_centers_arr = []
    sigma_arr = []
    for spec in tqdm(spectra_inorder):
        ...
        freq_center_LH, sigma_LH = plot_spectrum(loc, spec_LH, mod_LH, name_labels, freq_labels, figdir, plot_noise_range, save_figs, show_figs) # v_cen_labels
        freq_center_UH, sigma_UH = plot_spectrum(loc, spec_UH, mod_UH, name_labels, freq_labels, figdir, plot_noise_range, save_figs, show_figs) # v_cen_labels
...
def plot_spectrum(loc, spec, mod, name_labels, freq_labels, figdir, plot_noise_range, save_figs, show_figs): # v_cen_labels
    ...
    # Add labels
    axs[1].set_xlabel(f"Frequency [{(spec.xarr.to(u.GHz)).unit}]", fontsize = 13)
    axs[1].xaxis.set_major_locator(MultipleLocator(0.1))
    axs[1].xaxis.set_minor_locator(MultipleLocator(0.01))
    axs[0].set_ylabel(f"Brightness temperature [K]", fontsize = 13)
    axs[1].set_ylabel(f"Residual [K]", fontsize = 13)
    ...
    axs[0].legend(handles = [code_artist], labels = [code_label], loc="lower right", fontsize = 13)
    # Add line IDs
    if loc == 'core':
        maxspec.plotter.line_ids(name_labels, freq_labels*u.GHz, velocity_offset=40*u.km/u.s, plot_kwargs={'color':'grey', 'linewidth':0.5}) # v_cen currently overwritten
    elif loc == 'frown':
        maxspec.plotter.line_ids(name_labels, freq_labels*u.GHz, velocity_offset=12*u.km/u.s, plot_kwargs={'color':'grey', 'linewidth':0.5}) # v_cen currently overwritten
    if plot_noise_range:
