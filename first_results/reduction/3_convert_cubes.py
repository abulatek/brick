...
import dask
dask.config.set(scheduler = 'threads', num_workers = 1) # Used to be 8 but I don't understand parallelization
...
destination = '/blue/adamginsburg/abulatek/brick/symlinks/imaging_results/smoothed_cubes_K_w_pbcor/'
...
for freq_spw in freq_spws:
    fn_orig = 'source_ab_'+freq_spw+'_clean_2sigma_n50000_masked_3sigma_pbmask0p18.pbcor.image.fits'
    path_orig = results+fn_orig
    outfile = destination+fn_orig.replace('.pbcor.image.fits','.pbcor.image.commonbeam.K.fits')
    if not os.path.exists(outfile):
        # Import cube and convert it to a common beam
        cube = SpectralCube.read(path_orig, use_dask = True)
        cube.allow_huge_operations=True
        cube_K = cube.to(u.K) # Convert to K
        cube_common_beam = cube_K.beams.common_beam(max_iter = 20, max_epsilon = 0.01)
        smoothed_cube = cube_K.convolve_to(cube_common_beam) # Convert from VaryingResolution to regular
        # Write the new cube
        smoothed_cube.write(outfile, format='fits')