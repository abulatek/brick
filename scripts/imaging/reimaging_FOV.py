...
# Grab variables from command line arguments
...
image_prefix = output_dir+'source_ab_'+freq_num+'_spw'+spw+'_reimaging_FOV'
...
def make_dirty_map(data_path, image_prefix, spw, field):
    ...
    tclean(vis=data_path, imagename=dirty_map_image, field=field, spw=spw, specmode='cube', gridder='standard', uvrange = '0~100klambda', 
...
def deep_clean(data_path, output_dir, image_prefix, spw, sigma, nsigma, mask_image, niter, field):
    ...
    tclean(vis=data_path, imagename=cleaned_image, field=field, spw=spw, specmode='cube', gridder='standard', uvrange = '0~100klambda',
           cell=['0.2arcsec'], imsize=[512,512], weighting='natural', threshold=str(round(nsigma*sigma,1))+'mJy', mask=mask_image+'.mask',
           pbmask=0.05, niter=niter, interactive=False, chanchunks=-1)
