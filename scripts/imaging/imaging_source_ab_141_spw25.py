...
def deep_clean(data_path, output_dir, image_prefix, sigma, nsigma, mask_image, niter):
    cleaned_image = image_prefix+'_clean_'+str(nsigma)+'sigma_n'+str(niter)+'_masked_3sigma_pbmask0p18' # Do not add .image because CASA just wants prefix
...
cleaned_image = deep_clean(data_path, output_dir, image_prefix, sigma, 2., mask_image, 50000)
